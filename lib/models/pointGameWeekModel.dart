...
class PointGameWeek {
  ...
  Map<String, dynamic> toJson() => {
    ...
    "points": points,
    "keeper": keeper,
    "defender": List<String>.from(defender.map((x) => x)),
    "midfielder": List<String>.from(midfielder.map((x) => x)),
    "attacker": List<String>.from(attacker.map((x) => x)),
    "subKeeper": subKeeper,
    "substitution": List<String>.from(substitution.map((x) => x)),
    "_id": id,
    "__v": v,
  };
}
